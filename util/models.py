...
class Poll(Base):
    ...
    finalized_on:Mapped[datetime] = mapped_column(sa.DateTime(timezone=True), server_default=sa.text("'infinity'::timestamp"))
...
class PollOption(Base):
    ...
    bets:Mapped[List['Bet']] = relationship(back_populates='option', cascade='all, delete-orphan', lazy='joined')
    poll:Mapped['Poll'] = relationship(back_populates='options', cascade='all', lazy='joined', innerjoin=True)
